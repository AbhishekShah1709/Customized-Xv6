...
int flag = 0; //flag to check if initialized
int ticks_arr[5]={1,2,4,8,16};
        ...
        for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
            ...
            else if(p->state==RUNNABLE)
            {
                if(highP!=0)
                {
                    ...
                    else if(highP->priority == p->priority)
                    {
                        if(highP->last_executed > p->last_executed)
                        {
                            highP = p;
                        }
                    }
...
//            p = highP;
...
#else
#ifdef MLFQ
        int latest_ageing=ticks;
        struct proc *p=0;
        acquire(&ptable.lock);
        int level;


        for (level = 0, p = 0; level < 5; level++)
        {
            int i = ptable.queue_head[level];
            while( i != ptable.queue_tail[level] )
            {
                if(ptable.queue[level][i])
                {
                    p = ptable.queue[level][i];
                    break;
                }
                i = (i + 1) % NPROC;
            }
        ...
        {
            if (p->state != RUNNABLE)
                panic("process sleeping");
       
            remove (p->queue);
            
            int i=0;
            //For ensuring ticks for each cycle
            while( i < ticks_arr[p->queue] )
            {
            	if(p->state==RUNNABLE)
            	{
                	p->last_executed = ticks;
        ...
        if (ticks - latest_ageing > STARVATION_VAL)
        {
            for (int level = 1; level <= 4; level++)
            {
                for (int i = ptable.queue_head[level]; i != ptable.queue_tail[level]; i = (i + 1) % NPROC)
                {
                    p = ptable.queue[level][i];

                    if (ticks > STARVATION_VAL + p->last_executed)
                    {
                        remove (level);
                        add(p->queue - 1, p);
                    }
...
{
    //cprintf("myproc()->state %d\n",myproc()->state);
    //cprintf("myproc()->pid %d\n",myproc()->pid);
    //cprintf("myproc()->rtime %d\n",myproc()->rtime);
    //cprintf("myproc()->ticks[0] %d\n",myproc()->ticks[0]);
